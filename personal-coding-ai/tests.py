...
# ---------------------------------------------------------------------------
# Phase 3 Repair – failure-specific root cause & classification
# ---------------------------------------------------------------------------

def test_nameerror_is_reference_not_nullability():
    from debug_engine import run_debug, FailureType
    session = run_debug({
        "source_code": "print(x)",
        "error_message": "NameError: name 'x' is not defined",
    ...
    assert session.failure_type == FailureType.REFERENCE
    assert session.root_cause is not None
    summary = (session.root_cause.summary or "").lower()
    assert "null" not in summary and "undefined guard" not in summary
    assert any(k in summary for k in ("identifier", "name", "bound", "defined", "resolution"))


def test_zerodivision_specific_root_cause():
    from debug_engine import run_debug
    session = run_debug({
        "source_code": "print(1/0)",
        "error_message": "ZeroDivisionError: division by zero",
    ...
    summary = (session.root_cause.summary if session.root_cause else "").lower()
    assert "zero" in summary or "denominator" in summary
    assert "null" not in summary


def test_referenceerror_js_is_reference():
    from debug_engine import run_debug, FailureType
    session = run_debug({
        "source_code": "console.log(x)",
...
def test_syntax_is_syntax():
    ...
    assert session.failure_type == FailureType.SYNTAX
...
def test_hypothesis_requires_evidence():
    ...
    for h in session.hypotheses or []:
        if h.confidence_level.value != "INSUFFICIENT_EVIDENCE":
            assert h.supporting_evidence_ids, f"Hypothesis without evidence: {h.label}"


def test_language_propagates_to_pipeline():
    from debug_engine import run_debug
    session = run_debug({
        "source_code": "print(1)",
        "error_message": "NameError: name 'x' is not defined",
    ...
    assert session.input.language == "python" or (
        session.input.files and session.input.files[0].get("language") == "python"
    )


def test_learning_stores_and_retrieves():
    import tempfile, os
    from learning import LearningBrain
    from debug_engine import run_debug
    fd, path = tempfile.mkstemp(suffix=".db")
    os.close(fd)
    try:
        brain = LearningBrain(db_path=path)
        session = run_debug({
            "source_code": "print(x)",
            "error_message": "NameError: name 'x' is not defined",
            "language": "python",
            "user_description": "undefined variable x in python",
        })
        # force learn with this brain
        exp = brain.learn_from_session(session)
        assert exp is not None
        mems = brain.retrieve_for_debug(
            language="python",
            error_message="NameError: name 'x' is not defined",
...
def test_duplicate_experience_consolidated():
    import tempfile, os
    from learning import LearningBrain
    from debug_engine import run_debug
    fd, path = tempfile.mkstemp(suffix=".db")
    ...
    try:
        ...
        payload = {
            ...
            "error_message": "NameError: name 'x' is not defined",
            "language": "python",
            "user_description": "undefined x",
        }
        s1 = run_debug(payload)
        e1 = brain.learn_from_session(s1)
        s2 = run_debug(payload)
        e2 = brain.learn_from_session(s2)
        assert e1 is not None and e2 is not None
        # after consolidation count should not explode
